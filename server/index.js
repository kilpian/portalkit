...
async function renderVideo(framesDir, audioPath, outputPath, fps = 30, bgVideoPath = null) {
  ...
  return new Promise((resolve, reject) => {
    ...
    if (bgVideoPath && fs.existsSync(bgVideoPath)) {
      // Input 0: Pexels background video (looped)
      cmd.input(bgVideoPath)
         .inputOption('-stream_loop -1')

      // Input 1: Canvas frames (transparent PNG overlay)
      cmd.input(globPattern)
         .inputOptions(['-pattern_type glob', '-framerate ' + fps])

      if (hasAudio) cmd.input(audioPath)

      // Scale Pexels to portrait, darken 50%, overlay canvas frames
      const filter = [
        '[0:v]scale=1080:1920:force_original_aspect_ratio=increase,crop=1080:1920[bgscaled]',
        '[bgscaled]colorchannelmixer=rr=0.5:gg=0.5:bb=0.5[bgdark]',
      ...
      if (hasAudio) {
        cmd.outputOption('-map 2:a')
           .outputOption('-shortest')
      }
      cmd.outputOption('-t 30')
    } else {
      // Fallback: flat navy background generated by FFmpeg (always correct color)
      ...
      cmd.complexFilter('[0:v][1:v]overlay=0:0:format=rgb[vout]')
         .outputOption('-map [vout]')
      if (hasAudio) {
        cmd.outputOption('-map 2:a')
           .outputOption('-shortest')
