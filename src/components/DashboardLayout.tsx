...
function NavItem({ to, end, label, icon, onClick }: {
  to: string; end?: boolean; label: string; icon: React.ReactNode; onClick?: () => void
}) {
  return (
    <NavLink
      to={to}
...
export default function DashboardLayout() {
  ...
  const SidebarInner = () => (
    <>
      {/* Logo */}
      <div style={{ padding: '18px 20px', borderBottom: '1px solid rgba(255,255,255,0.08)', flexShrink: 0 }}>
      ...
      {/* Settings + User */}
      <div style={{ borderTop: '1px solid rgba(255,255,255,0.08)', padding: '10px 10px 4px', flexShrink: 0 }}>
        <NavItem
      ...
      <div style={{ padding: '12px 14px 16px', borderTop: '1px solid rgba(255,255,255,0.06)' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 10 }}>
          {isActive ? (
            <span style={{ fontSize: 11, fontWeight: 600, padding: '2px 9px', borderRadius: 99, background: 'rgba(52,211,153,0.16)', color: '#34D399', border: '1px solid rgba(52,211,153,0.32)' }}>
              Active
            </span>
          ) : (
            <>
              <span style={{ fontSize: 11, fontWeight: 600, padding: '2px 9px', borderRadius: 99, background: 'rgba(201,168,76,0.16)', color: '#C9A84C', border: '1px solid rgba(201,168,76,0.32)' }}>
                Trial
              </span>
              {days > 0 && (
                <span style={{ fontSize: 11, color: 'rgba(255,255,255,0.38)' }}>{days}d remaining</span>
              )}
              {days === 0 && (
                <span style={{ fontSize: 11, color: 'rgba(239,68,68,0.8)' }}>Expired</span>
              )}
            </>
          )}
        </div>
