...
export default function Files() {
  ...
  // Visible files by sidebar selection
  ...
  const catCounts = (['All', 'Photos', 'Videos', 'Documents', 'Other'] as const).map(cat => ({
    cat,
    count: cat === 'All' ? sidebarFiles.length : sidebarFiles.filter(f => getCategory(f.mime_type) === cat).length,
  }))
          ...
          {[
            ...
            <button
              ...
              style={{
                ...
                padding: '10px 14px',
                background: sidebarKey === key ? 'var(--bg-secondary)' : 'transparent',
                border: 'none',
                borderBottom: '1px solid var(--border-subtle)',
                cursor: 'pointer',
                textAlign: 'left',
                fontWeight: sidebarKey === key ? 700 : 400,
                color: sidebarKey === key ? 'var(--green)' : 'var(--text-primary)',
                fontSize: 13,
                          ...
                          <div style={{ padding: '6px 8px', background: 'linear-gradient(to top, rgba(0,0,0,0.7), transparent)' }}>
                            ...
                            <div style={{ display: 'flex', gap: 4 }}>
                              <a
                                ...
                                style={{ fontSize: 10, padding: '2px 6px', borderRadius: 4, background: 'rgba(255,255,255,0.2)', color: '#fff', textDecoration: 'none', fontWeight: 600 }}
                              >↓</a>
                              <button
                                onClick={e => { e.stopPropagation(); deleteFile(f.id, f.original_name) }}
                                disabled={deleting === f.id}
                                style={{ fontSize: 10, padding: '2px 6px', borderRadius: 4, background: 'rgba(220,38,38,0.6)', border: 'none', color: '#fff', cursor: 'pointer', fontWeight: 600 }}
                              >✕</button>
                            </div>
                          </div>
                        </div>
