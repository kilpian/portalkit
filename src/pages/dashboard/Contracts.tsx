...
export default function Contracts() {
  ...
  const photographerFullName = user?.full_name || ''
  ...
  const renderedContent = DOMPurify.sanitize(String(marked.parse(form.content || '')))
  ...
  const downloadSignatureRecord = (c: Contract) => {
    ...
    const content = `
      ...
      <body>
        ...
        <div class="field"><div class="label">Contract</div><div class="value">${escapeHtml(c.title)}</div></div>
        <div class="field"><div class="label">Reference</div><div class="value">${escapeHtml(c.content_hash?.slice(-8).toUpperCase() ?? '—')}</div></div>
        <div class="contract-text">${escapeHtml(c.content ?? '')}</div>
        <div class="sig-grid">
          <div class="sig-block">
            <p style="font-weight:bold;margin-bottom:16px;">PHOTOGRAPHER</p>
            <div style="font-family: 'Dancing Script', cursive; font-size: 28px; color: #1B4332; border-bottom: 1px solid #333; padding-bottom: 4px; margin-bottom: 4px;">${escapeHtml(c.photographer_signature || businessName)}</div>
            <div style="font-size: 11px; color: #666;">${escapeHtml(businessName)}${photographerFullName ? ' · ' + escapeHtml(photographerFullName) : ''}</div>
            <p style="font-size:12px;margin-top:8px;">Date: ${photographerDate || '_______________'}</p>
          </div>
          <div class="sig-block">
            <p style="font-weight:bold;margin-bottom:16px;">CLIENT</p>
            <div style="font-family: 'Dancing Script', cursive; font-size: 28px; color: #1B4332; border-bottom: 1px solid #333; padding-bottom: 4px; margin-bottom: 4px; min-width: 200px;">${escapeHtml(c.signed_by_name ?? '—')}</div>
            <div style="font-size: 11px; color: #666;">Electronic signature${clientDate ? ' · ' + clientDate : ''}${c.content_hash ? ' · Ref: ' + escapeHtml(c.content_hash.slice(-8).toUpperCase()) : ''}</div>
          </div>
        </div>
  ...
  return (
    <div style={{ padding: 'clamp(16px, 4vw, 32px) clamp(16px, 4vw, 32px) 64px', maxWidth: 900, margin: '0 auto' }}>
      ...
      {loading ? (
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
          {contracts.map(c => (
            <div key={c.id} style={{ background: '#fff', border: '1px solid var(--border)', borderRadius: 10, padding: '14px 18px', display: 'flex', alignItems: 'center', flexWrap: 'wrap', gap: 10 }}>
        ...
        <>
          ...
          <div style={{ position: 'fixed', top: '50%', left: '50%', transform: 'translate(-50%,-50%)', zIndex: 300, width: 'min(420px, 90vw)', background: 'var(--bg-elevated)', borderRadius: 14, boxShadow: '0 20px 60px rgba(0,0,0,0.2)', padding: 28 }}>
            ...
            <p style={{ fontSize: 12, color: 'var(--text-dim)', marginBottom: 8, lineHeight: 1.5 }}>
              Your signature will appear as your typed name in an official cursive style.
            </p>
